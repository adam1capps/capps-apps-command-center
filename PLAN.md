...
Open follow-ups (not blocking the live site): set 3 still-missing Netlify secrets per-context (`GITHUB_TOKEN` rotation, `X_TRIGGER_TOKEN`, `CAPPSHUB_HOOK_TOKEN`) for the Phase 4 poller + Phase 12. `CAPPSHUB_WEBHOOK_SECRET` is done (2026-05-28, secret per-context, exercised by hub-dispatch). Runtime-verification debt: the public showcase `/` is confirmed live (cards render from the DB at `hub.cappsapps.ai`, 2026-05-21), but the authenticated notes CRUD/autosave flows still want a click-through pass on `/app/[slug]`.
