...
### Current phase
**Phase 11 (GitHub push webhook → cache invalidation)** — code DONE/merged (PR #31 merge `42728d4`, 2026-05-22). `/api/github-webhook` (HMAC-verified against `CAPPSHUB_WEBHOOK_SECRET`) invalidates `notes_cache` rows for pushed `.cappshub/*` or `CLAUDE.md` files on the default branch and records a `push` row in `integration_events` (new table, pre-created here; Phase 12 owns its main use). Migration `0004` + `bundle-0004.sql`; `cappshub.ts` gained `resolveAppByRepo` + `invalidatePaths`; `app/scripts/install-webhooks.ts`/`uninstall-webhooks.ts` + `pnpm webhooks:install|uninstall [filter]` (run locally, idempotent, optional repo-name filter to stage). **Runtime status:** `bundle-0004.sql` applied to Neon 2026-05-22 (`integration_events` exists, count=0, schema verified column-by-column). Only remaining blocker: install the webhook on at least one managed repo — user chose the local-toolchain path (`pnpm webhooks:install`); their Mac needs a clone + Node/pnpm first. Endpoint is dormant until hooks are installed. Next build step: **Phase 12A** (`/api/cappshub-events` POST + SSE stream).
...
| 11 | GitHub webhook | code done (merged); webhook install pending | PR #31 `42728d4` (github-webhook + integration_events + invalidatePaths + install scripts) | 2026-05-22T00:10Z · green · build gate green; deploy `6a0fb8e3` ready | `bundle-0004.sql` applied to Neon 2026-05-22 (`integration_events` exists, count=0, schema verified). Remaining: `pnpm webhooks:install` (user, local toolchain); endpoint dormant until at least one repo has the hook installed |
