...
### Current phase
**Phase 10 (`.cappshub/` GitHub Contents reads)** — DONE. 10A shipped (PR #27 merge `c2f50fd`, 2026-05-21): `notes_cache` table + `bundle-0003.sql`, `github.ts` `getRepoContents`/`listDirContents`, and `cappshub.ts` cache-aware reads (5-min freshness, miss-caching). 10B shipped (PR #29 merge `b82af64`, 2026-05-21): `IntegrationCard` wired to real cappshub reads via `getIntegrationFor` (4 tabs: instructions/plan/notes/CLAUDE.md, notes history nav, "no .cappshub/" empty state + non-functional "Create one"), plan card prefers `.cappshub/plan.json` with a "synced from .cappshub/plan.json" label. Live: `bundle-0003.sql` applied to Neon (`notes_cache` exists), `GITHUB_TOKEN` set + valid, both DB creds rotated + hardened + production deploy `ready` on rotated secrets. Most managed repos have no `.cappshub/` yet so empty states are expected (dogfood lands Phase 13). Next: **Phase 11A** — GitHub push webhook → cache invalidation (has a user-decision point: install webhook across all managed repos vs. stage to a subset).
