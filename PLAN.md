...
### Current phase
**Phase 12 (cappshub-events POST + SSE + SyncToast + /integration)** — DONE 2026-05-28. 12A shipped in PR #33 (merge `af8ba5a`): `POST /api/cappshub-events` Bearer-guarded against `CAPPSHUB_HOOK_TOKEN` via `node:crypto.timingSafeEqual`, zod-validated body with `kind` enum `push | note | instruct | plan | sync`, slug → `app_id` (404 on miss), text sliced to 280 chars, 204 on success; `GET /api/events/stream` Clerk-gated SSE that polls `integration_events WHERE created_at > $lastSeen` every 1.5s, 30s `: ping` heartbeats, `AbortSignal` cleanup. Both runtime `nodejs`. Polling-over-broadcaster rationale documented in the route header. 12B shipped in PR #34 (merge `9fe5a53`): `SyncToast` (EventSource subscriber mounted in `DashboardClient`, 5.2s display, click navigates to `/app/[slug]`); `/integration` page (Clerk-gated via the `proxy.ts` matcher addition, 6 sections: hero / 4-stat strip / slash command cards / hook-config copy block / `.cappshub/` folder spec / live event feed / connected repos); `app/lib/cappshub-spec.ts` (typed port of SLASH_COMMANDS + HOOK_CONFIG from `prototype/cc-integration.js:21-85`); three new db-query helpers (`getRecentEvents`, `getConnectedRepos`, `getIntegrationSummary`). No schema change in either PR. Next build step: **Phase 13** (slash/hook docs + this-repo dogfood; user-gated on hook install on the command-center repo itself).
...
Open follow-ups (not blocking the live site): 2 still-missing/debt Netlify secrets — `GITHUB_TOKEN` (functional but stored non-secret + leaked; rotate to true secret per-context) and `X_TRIGGER_TOKEN` (Phase 4 manual-trigger guard, never provisioned). `CAPPSHUB_WEBHOOK_SECRET` is done (2026-05-28, secret per-context, exercised by hub-dispatch). `CAPPSHUB_HOOK_TOKEN` is done (2026-05-28, true secret per-context, provisioned for Phase 12A; user has the value in 1Password). Runtime-verification debt: Phase 12A POST + SSE not yet smoke-tested live (curl + EventSource snippets are in PR #33's body); Phase 12B `/integration` page + dashboard `SyncToast` not yet eyeballed live (test plan in PR #34's body); authenticated notes CRUD/autosave flows still want a click-through pass on `/app/[slug]`.

SCP 2026-05-28T19:45Z · green · `implement-command-center` @ `9fe5a53` (PR #34 / Phase 12B): squash-merge of `SyncToast` (EventSource subscriber on dashboard) + `/integration` page (Clerk-gated, 6 sections: hero / 4-stat strip / slash commands / hook config / `.cappshub/` spec / live feed / connected repos) + `cappshub-spec.ts` + 3 db-query helpers (`getRecentEvents`, `getConnectedRepos`, `getIntegrationSummary`) + `proxy.ts` matcher addition. install/typecheck/lint/build all pass; `/integration` registers as a dynamic route. Netlify MCP 502 at SCP time (sandbox-side) so deploy state not directly verified; Netlify auto-deploys on push to `implement-command-center` so a new deploy is in flight. Phase 12 fully done. Live runtime smoke pending the user's laptop (test plan in PR #34's body).
...
| 12 | `/api/cappshub-events` + SSE + SyncToast + `/integration` | done | 12A: PR #33 `af8ba5a` (POST + SSE stream) · 12B: PR #34 `9fe5a53` (SyncToast + `/integration` + cappshub-spec + db helpers + proxy matcher) | 12B: 2026-05-28T19:45Z · green · build gate green; `/integration` registers as dynamic; Netlify MCP 502 at SCP time so deploy state not verified from sandbox | runtime smoke test of 12A POST + EventSource + 12B toast/feed still owed (user's laptop) |
