// App Intel - deterministic synthetic metrics per app, ported verbatim from
// prototype/app-intel.js. Values are derived from a hash of the app slug so a
// given app always renders the same numbers. Each generator is marked with the
// real data source that will replace it in a later phase.

import { SEED_NOW } from "./derive";
...
// TODO(real-data): GitHub commit-activity API.
function commitsTimeline(
  ...
  const totalLo =
    app.stage === "mature" ? 80 : app.stage === "live" ? 40 : app.stage === "mvp" ? 14 : 6;
  const totalHi =
    app.stage === "mature" ? 240 : app.stage === "live" ? 120 : app.stage === "mvp" ? 38 : 18;
  const total = rint(r, totalLo, totalHi);
  const series: number[] = [];
  let remaining = total;
  for (let i = 0; i < weeks; i++) {
    const bias =
      app.stage === "archive" || !app.githubRepo
        ? Math.max(0, 1 - i / 3)
        : 0.4 + 0.6 * (i / weeks);
    const share = bias * r();
  ...
  if (!app.githubRepo) return { total: 0, weekly: new Array(weeks).fill(0) };
  return { total, weekly: series };
}

const LANG_SETS: Record<string, [string, number][]> = {
...
// TODO(real-data): GitHub languages API.
function languages(app: IntelInput): Language[] {
  ...
  else if (app.slug.includes("smart-bookkeeper") || app.slug.includes("gravimetric"))
    set = LANG_SETS.py;
  else if (app.slug.includes("mindreadir") || app.slug.includes("proposal-builder"))
    set = LANG_SETS.mixed;
  else if (app.slug.includes("website") || app.slug.includes("guide") || app.slug.includes("links"))
    set = LANG_SETS.web;
  else set = LANG_SETS.react;
  return set.map(([name, pct]) => ({ name, pct }));
}

const TABLE_DICT: [string, number, number][] = [
  ["users", 50, 800],
  ["sessions", 200, 4000],
  ["audit_log", 800, 30000],
  ["events", 400, 12000],
  ["api_requests", 2000, 80000],
];
const NAMED_TABLES: Record<string, string[]> = {
...
// TODO(real-data): Neon schema introspection + row counts.
function database(app: IntelInput, r: () => number, daysAgo: (d: number) => Date): Database | null {
  if (!app.apiDependencies.includes("Neon")) return null;
  const named = NAMED_TABLES[app.slug] ?? null;
  const tableNames =
    named ?? Array.from({ length: rint(r, 2, 5) }, () => pick(r, TABLE_DICT)[0]);
  const tables: DbTable[] = tableNames.map((name, i) => {
    ...
    return {
      ...
      lastWrite: daysAgo(rint(r, 0, app.stage === "mature" ? 5 : 30)),
    };
  });
  const totalRows = tables.reduce((s, t) => s + t.rows, 0);
  return {
    name: `${app.slug.replace(/-/g, "_")}_prod`.slice(0, 38),
    region: "aws-us-east-2",
    tables,
    totalRows,
    sizeMb: Math.round((totalRows / 1800) * 10) / 10,
  };
}

// TODO(real-data): Netlify deploy history API.
function deploys(app: IntelInput, r: () => number, daysAgo: (d: number) => Date): Deploy[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const branch =
      i === 0
        ...
        : r() < 0.85
          ? "main"
          : "feat/" +
            pick(r, ["tighten-copy", "fix-pdf", "rate-limit", "ui-polish", "neon-pool", "ssl-cert"]);
    const status: Deploy["status"] =
      (i === 0 && app.urlStatus === 500) || (i === 0 && app.urlStatus === 404)
        ? "failed"
        : r() < 0.04
          ? "failed"
          : "succeeded";
    out.push({
      ...
      status,
      durationS: rint(r, 35, 240),
    });
  }
  return out;
}

const PLAN_TEMPLATES: Record<string, { done: boolean; text: string }[]> = {
  "proposals-roofmri": [
  ...
  "warranty-management": [
    { done: false, text: "Upgrade Render free to paid tier (cold starts 1.8s)" },
  ...
  "roof-mri-website": [{ done: false, text: "Refresh case-study section with Rackley data" }],
  "training-site": [{ done: false, text: "Decide: redirect to protraining or rehost in a repo" }],
  "moisture-detection": [{ done: false, text: "Move source into a repo (currently Netlify-only)" }],
  "gravimetric-readings": [
    { done: false, text: "CSV import from lab USB scale" },
    { done: false, text: "Calibration regression chart" },
  ],
  "smart-bookkeeper": [
    { done: false, text: "Rule engine prototype to ship as v0" },
    { done: false, text: "Fold into ReDry Accounting" },
  ],
  "linkedin-app": [{ done: false, text: "Resume once LinkedIn API access lands" }],
  taskline: [{ done: false, text: "Decide if this stays personal" }],
  "hub-app-scanner": [
    { done: false, text: "Push results to Neon for the Command Center" },
    { done: true, text: "Detect Netlify-only deploys" },
  ],
};
// TODO(real-data): .cappshub/plan.json via the GitHub Contents API (Phase 10).
function plan(app: IntelInput): PlanItem[] {
  const blueprint = PLAN_TEMPLATES[app.slug];
  if (blueprint) return blueprint.map((p, i) => ({ id: `${app.slug}-p${i}`, ...p }));
  return [{ id: `${app.slug}-p0`, done: false, text: app.nextMove || "Define the next move." }];
}

// TODO(real-data): provider usage metrics.
function apiUsage(app: IntelInput, r: () => number): ApiUsage[] {
  return app.apiDependencies.map((name) => ({
    name,
    calls: rint(r, 12, name === "Neon" ? 5200 : 600),
    fails: r() < 0.08 ? rint(r, 1, 4) : 0,
    p95Ms: rint(r, 80, name === "Anthropic" ? 2400 : 800),
  }));
}

// TODO(real-data): Plausible 7-day traffic.
function traffic(app: IntelInput, r: () => number): Traffic | null {
  if (!app.liveUrl) return null;
  const base =
    app.stage === "mature"
      ? rint(r, 600, 3500)
      : app.stage === "live"
  ...
  const week = Array.from({ length: 7 }, () =>
    Math.max(0, Math.round((base / 7) * (0.6 + r() * 0.9))),
  );
  return {
    sessions: week.reduce((a, b) => a + b, 0),
    week,
    referrers: [
      { src: "direct", pct: 42 },
      { src: "google", pct: 31 },
...
// TODO(real-data): Netlify site config + env var names.
function hostingDetail(
  app: IntelInput,
  r: () => number,
  daysAgo: (d: number) => Date,
): HostingDetail | null {
  if (!app.hosting || app.hosting === "none") return null;
  const envs: string[] = [];
  const probable = new Set<string>();
  const deps = app.apiDependencies;
  if (deps.includes("Anthropic")) probable.add("ANTHROPIC_API_KEY");
  if (deps.includes("Neon")) probable.add("DATABASE_URL");
  if (deps.includes("SendGrid")) probable.add("SENDGRID_API_KEY");
  if (deps.includes("Clerk")) probable.add("CLERK_SECRET_KEY");
  if (deps.includes("Stripe")) probable.add("STRIPE_SECRET_KEY");
  if (deps.includes("Plaid")) {
    probable.add("PLAID_CLIENT_ID");
    probable.add("PLAID_SECRET");
  }
  if (deps.includes("Render")) probable.add("RENDER_SERVICE_ID");
  if (deps.includes("Mapbox")) probable.add("MAPBOX_TOKEN");
  if (app.liveUrl) probable.add("NEXT_PUBLIC_BASE_URL");
  for (const n of probable) envs.push(n);
  while (envs.length < probable.size + (r() < 0.5 ? 1 : 0)) {
    const remaining = ENV_NAMES.filter((n) => !envs.includes(n));
    if (remaining.length === 0) break;
    envs.push(pick(r, remaining));
  }
  return {
    provider: app.hosting,
    siteId: `${app.slug}-${hash(app.slug).toString(16).slice(0, 6)}.netlify.app`,
    branch: "main",
    buildCmd: app.slug.includes("py") ? "uv run build" : "npm run build",
    envVarNames: envs.slice(0, 8),
    sslExpires: daysAgo(-rint(r, 30, 180)),
  };
}

export function intelFor(app: IntelInput, now: Date = SEED_NOW): AppIntel {
  const r = rng(hash(app.slug));
  const daysAgo = (d: number) => new Date(now.getTime() - d * 86_400_000);
  return {
    commits: commitsTimeline(app, r),
    languages: languages(app),
    database: database(app, r, daysAgo),
    deploys: deploys(app, r, daysAgo),
    plan: plan(app),
    apiUsage: apiUsage(app, r),
    traffic: traffic(app, r),
    hosting: hostingDetail(app, r, daysAgo),
    contributors: app.githubRepo ? Math.min(3, 1 + (hash(app.slug) % 3)) : 0,
  };
}
